...
- [ ] CI: contract simulator tests + frontend build + **coverage gate** + lint/format/type-check gate + markdown lint + frontend image build
  - *In progress*: the pure-Node `test` workflow (contract offline tests + coverage gate) is done; frontend build/coverage, the localnet integration lane, and image build remain.
